...
import {TriggerContainer} from '~/src/sections/binance/futures/triggers/trigger-container'
import {TriggerListTable} from '~/src/sections/binance/futures/triggers/trigger-list-table'
import {TriggerListSearch} from '~/src/sections/binance/futures/triggers/trigger-list-search'
import PlusIcon from '@untitled-ui/icons-react/build/esm/Plus'
import {useMounted} from '~/src/hooks/use-mounted'
import {triggersApi} from '~/src/api/cryptos/binance/futures/triggers'
import {Trigger} from '~/src/types/binance/futures/trigger'
...
const useListings = (searchState: TriggersSearchState) => {
  ...
  const [state, setState] = useState<TriggerStoreState>({
    triggers: [],
  })
  ...
  const listings = useCallback(async () => {
    try {
      ...
      if (isMounted()) {
        ...
        setState({
          triggers: (response.data ?? []) as Trigger[],
        })
      }
    } catch (err) {
      console.error(err)
...
const Page:NextPageWithLayout = () => {
  ...
  const { triggers } = useListings(search)
  ...
  return (
    <>
      <Head>
        <title>
          Dashboard: Trigger List | Devias Kit PRO
        </title>
      </Head>
      <Divider />
      <Box
