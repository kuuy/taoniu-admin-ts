...
const statusMap: Record<TradingStatus, string> = {
  canceled: 'warning',
  complete: 'success',
  pending: 'info',
  rejected: 'error'
...
export const TradingDetails: FC<TradingDetailsProps> = (props) => {
  const { onApprove, onEdit, onReject, trading } = props;
  const lgUp = useMediaQuery((theme: Theme) => theme.breakpoints.up('lg'));

  const align = lgUp ? 'horizontal' : 'vertical';
  const items = trading.items || [];
  const createdAt = format(trading.createdAt, 'dd/MM/yyyy HH:mm');
  const statusColor = statusMap[trading.status] as SeverityPillColor;
        ...
        <PropertyList>
          ...
          >
            ...
            <Typography
              ...
              variant="body2"
            >
              {trading.customer.address1}
            </Typography>
            <Typography
              color="text.secondary"
              variant="body2"
            >
              {trading.customer.city}
            </Typography>
            <Typography
              color="text.secondary"
              variant="body2"
            >
              {trading.customer.country}
            </Typography>
          </PropertyListItem>
          <PropertyListItem
            align={align}
